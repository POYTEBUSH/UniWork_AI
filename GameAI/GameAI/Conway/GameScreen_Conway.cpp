...
void GameScreen_Conway::UpdateMap()
{
	...
	for (int i = 0; i < (kConwayScreenHeight / kConwayTileDimensions); i++)
	{
		memset(mNextMap[i], 0, (kConwayScreenHeight / kConwayTileDimensions) * sizeof(mNextMap[i][0]));
	}
	...
	for (int y = 0; y < (kConwayScreenHeight / kConwayTileDimensions); y++)
	{
		for (int x = 0; x < (kConwayScreenWidth / kConwayTileDimensions); x++)
		{
			...
			//We now know the number of adjacent living cells
			if (mMap[x][y] == 1) {
...
int GameScreen_Conway::GetAdjacentCells(int x, int y) {
	...
	if (mMap != nullptr) {
		...
		if (x - 1 >= 0 && x - 1 < (kConwayScreenWidth / kConwayTileDimensions) && y - 1 >= 0 && y - 1 < (kConwayScreenHeight / kConwayTileDimensions))
			output += mMap[x - 1][y - 1];

		if (x - 1 >= 0 && x - 1 < (kConwayScreenWidth / kConwayTileDimensions) && y >= 0 && y < (kConwayScreenHeight / kConwayTileDimensions))
			output += mMap[x - 1][y];

		if (x - 1 >= 0 && x - 1 < (kConwayScreenWidth / kConwayTileDimensions) && y + 1 >= 0 && y + 1 < (kConwayScreenHeight / kConwayTileDimensions))
			output += mMap[x - 1][y + 1];

		if (x >= 0 && x < (kConwayScreenWidth / kConwayTileDimensions) && y - 1 >= 0 && y - 1 < (kConwayScreenHeight / kConwayTileDimensions))
			output += mMap[x][y - 1];
